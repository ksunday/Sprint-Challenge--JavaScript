...
zooAnimals.forEach(names =>
  animalNames.push(`${names.animal_name} ${names.scientific_name}`)
);
...
const lowerCase = [];
zooAnimals.map(lc => lowerCase.push(lc.animal_name.toLowerCase()));
...
*/
const lowerPopulation = zooAnimals.filter(popul => {
  return popul.population < 5;
});
